...
// sbit OLED_SCL=P5^3; //K60的管脚配置  D0 1.0
// sbit OLED_SDA=P5^2;                //
// sbit OLED_RST=P5^0;               //
// sbit OLED_DC=P5^1;                //
...
#define OLED_SCL_H      GPIOB->BSRR = GPIO_Pin_13  //时钟
#define OLED_SCL_L      GPIOB->BRR  = GPIO_Pin_13 

#define OLED_SDA_H      GPIOB->BSRR = GPIO_Pin_12 //数据
#define OLED_SDA_L      GPIOB->BRR  = GPIO_Pin_12

#define OLED_RST_H      GPIOB->BSRR = GPIO_Pin_11 //时钟
#define OLED_RST_L      GPIOB->BRR  = GPIO_Pin_11 

#define OLED_DC_H       GPIOB->BSRR = GPIO_Pin_10//片选	
#define OLED_DC_L       GPIOB->BRR  = GPIO_Pin_10
...
{
 1// "北京龙丘邱科技开发板智能车首选液晶室温度按键次电压"
};
static const unsigned char   F14x16[] = {  1
...
void OLED_Init(void)
{
  ...
  OLED_WrCmd(0xcf);
  OLED_WrCmd(0xa1);//--set segment re-map 95 to 0
  ...
  OLED_WrCmd(0xd5);//--set display clock divide ratio/oscillator frequency
  OLED_WrCmd(0x80);//--set divide ratio
  OLED_WrCmd(0xd9);//--set pre-charge period
  OLED_WrCmd(0xf1);
  OLED_WrCmd(0xda);//--set com pins hardware configuration
  OLED_WrCmd(0x12);
...
//==============================================================
//函数名：OLED_write_char(byte x,byte y,byte c)
//功能描述：写入一组标准ASCII字符串
...
{         	
	...
	for(t=1;t<=len;t++)
	{
		...
		OLED_write_char(x+6*t,y,temp+48);
	}
} 
    ...
    for (n=0;n<row;n++)        //换行
      {
        for(i=0;i<Pix_x;i++)
          {
                   OLED_Set_Pos(X+i,Y+n);
                   OLED_WrDat(map[i+n*Pix_x]);
          }
      }
}
...
void OLED_write_4_num(unsigned char X,unsigned char Y,signed int s)
{
	 ...
	 OLED_write_char(X+6,Y,(s/1000)+0x30);
	 OLED_write_char(X+12,Y,(s/100)%10+0x30);
	 OLED_write_char(X+18,Y,(s/10%10)+0x30);	
   OLED_write_char(X+24,Y,(s%10)+0x30);	
}
...
void OLED_write_3_num(unsigned char X,unsigned char Y,signed int s)
{
     ...
     OLED_write_char(X+6,Y,(s/100)+0x30);
     OLED_write_char(X+12,Y,((s%100)/10)+0x30);
     OLED_write_char(X+18,Y,(s%10)+0x30);	
 }

/*********************************************************************/
  void OLED_write_2_num(unsigned char X,unsigned char Y,signed int s)
  {
	 OLED_write_char(X,Y,(s/10)+0x30);
	 OLED_write_char(X+6,Y,(s%10)+0x30);	
  }

  void OLED_write_1_num(unsigned char X,unsigned char Y,signed int s)
  {
     OLED_write_char(X,Y,s+0x30);	
  }
...
void OLED_write_3_num_unsigned(unsigned char X,unsigned char Y,signed int s)
  {
     ...
     OLED_write_char(X,Y,(s/100)+0x30);
     OLED_write_char(X+6,Y,((s%100)/10)+0x30);
     OLED_write_char(X+12,Y,(s%10)+0x30);	
  }

void OLED_write_4_num_unsigned(unsigned char X,unsigned char Y,signed int s)
  {

     OLED_write_char(X,Y,(s/1000)+0x30);
     OLED_write_char(X+6,Y,((s%1000)/100)+0x30);
     OLED_write_char(X+12,Y,(s%100)/10+0x30);	
     OLED_write_char(X+18,Y,(s%10)+0x30);	
  }
...
void OLED_write_5_num_unsigned(unsigned char X,unsigned char Y,signed int s)
  {
     OLED_write_char(X,Y,(s/10000)+0x30);
     OLED_write_char(X+6,Y,(s%10000)/1000+0x30);
     OLED_write_char(X+12,Y,((s%1000)/100)+0x30);
     OLED_write_char(X+18,Y,(s%100)/10+0x30);	
     OLED_write_char(X+24,Y,(s%10)+0x30);	
  }
